...
def waitForMeterpreters(testConfig, sessionCounter):
    modCounter = 0
    msfDone = False
    loopCounter = 0
    msfConsoleCount = 1
    ...
    try:
        while not msfDone:
            ...
            logMsg(testConfig['LOG_FILE'], str(maxLoops-loopCounter) + " LOOPS REMAINING BEFORE AUTOMATIC EXIT")
...
def finishStageTwo(testConfig, terminationToken):
...
def waitForMsfPayloads(msfHosts, reportDir, logFile):
    """
...
def finishAndLaunchStageOne(msfHosts, httpPort):
...
def getSessionCounts(testConfig):
    ...
    for host in testConfig['TARGETS']:
        if host['TYPE'] == "VIRTUAL":
            if 'TESTING_SNAPSHOT' in host:
                ...
                logMsg(testConfig['LOG_FILE'], "TRYING TO REVERT TO " + host['TESTING_SNAPSHOT'])
